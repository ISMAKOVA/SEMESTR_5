...
def read_docs_with_class(classes_name):
    ...
    for class_name in classes_name:
        path = '/Applications/MAMP/htdocs/5 семестр/ВЕБ_7/лаба_7_Rocchio/news_yandex/' + class_name + '_all.txt'
        string = ""
        f = open(path)
...
def make_test_text(file):
    path = '/Applications/MAMP/htdocs/5 семестр/ВЕБ_7/лаба_7_Rocchio/test/' + file + '.txt'
    string = ""
    f = open(path)
    for line in f.readlines():
        string += line
...
if __name__ == '__main__':
    ...
    # write_centroids(centroids) #запись в файл centroids
    ...
    for text in texts:
        ...
        print()
    # compute_idf(class_docs, token_dict)